...
* Formatting of output 
  * Currently, a TwoWayPrice object is returned to convey the computed VWAP two-way price. I did not change this as per the task specifications. However, it would perhaps be clearer and more efficient to return a different type of object which is specific to a VWAP computation, rather than a part of a MarketUpdate. This would enable one to return only the 2 bid and offer values needed for a VWAP computation, as well as an indication of state (indicative/firm). This would be especially useful if further computations would need to be made with the returned VWAP value - a distinct class would allow for definition of VWAP-specific methods and any additional attributes as needed. From a legibility standpoint doing so would make the program clearer and easier to understand for an outside user.
